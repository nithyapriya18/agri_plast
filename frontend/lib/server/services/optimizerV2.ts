...
export class PolyhouseOptimizerV2 {
  ...
  async optimize(): Promise<Polyhouse[]> {
    ...
    if (currentCoverage < 85) {
      ...
      console.log(`\n🎯 SECOND PASS: Filling gaps with polyhouses ≥${minAreaForGapFill}m²...`);
      ...
      for (let row = 0; row < medRows; row++) {
        for (let col = 0; col < medCols; col++) {
          ...
          if (best) {
            ...
            if (secondPassCount % 2 === 0) console.log(`  +${secondPassCount} polyhouses (${currentCoverage.toFixed(1)}%)`);
          }
        }
    ...
    if (currentCoverage < 70) {
      console.log(`\n🎯 THIRD PASS: Filling remaining gaps with SMALL polyhouses...`);
          ...
          for (let col = 0; col < smallCols; col++) {
            ...
            if (best) {
              ...
              if (thirdPassCount % 3 === 0) {
                const cov = (polyhouses.reduce((sum, p) => sum + p.area, 0) / this.landArea.area) * 100;
                console.log(`  +${thirdPassCount} small polyhouses (${cov.toFixed(1)}%)`);
              }
            }
          }
      ...
      for (const restrictedZone of this.terrainData.restrictedAreas) {
        if (restrictedZone.type === 'water' && restrictedZone.severity === 'prohibited') {
          try {
            ...
            console.warn('Invalid restricted zone polygon');
          }
        }
      }
    }
